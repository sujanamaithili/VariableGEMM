...
#define ISCLOSE_RELTOL                                                         \
  1e-6 // this would not work for precision lower than float
#define ISCLOSE_ABSTOL 1e-6

// Index is a MACRO that returns the element of t tensor at row, col coordinate
#define Index(t, row, col)                                                     \
  ((((t).rawp)[(t).offset + (row) * (t).stride_h + (col) * (t).stride_w]))
// IndexOutofBound is a MACRO to test whether coordinate row,col is considered
// out of bounds for tensor "t"
...
template <typename T> struct cpuDeleter {
  void operator()(T *p) const {
    if (p != nullptr) {
      // std::cout << "Free p=" << p << std::endl;
      free(p);
...
public:
  ...
  Tensor()
      : h(0), w(0), stride_h(0), stride_w(0), offset(0), rawp(nullptr),
        on_device(false) {
  ...
  Tensor(int32_t h_, int32_t w_, bool on_device_ = false)
    ...
    if (on_device_) {
      ...
      // std::cout << "malloc p=" << rawp << " h=" << h << " w=" << w <<
      // std::endl;
      ref = std::shared_ptr<T>(rawp, cpuDeleter<T>());
    }
  }
  ...
  void toHost(Tensor<T> &out) const {
    ...
    cudaAssert(
        cudaMemcpy(out.rawp, rawp, h * w * sizeof(T), cudaMemcpyDeviceToHost));
  }
  ...
  void toDevice(Tensor<T> &out) const {
    ...
    out.stride_w = stride_w;
    cudaAssert(
        cudaMemcpy(out.rawp, rawp, h * w * sizeof(T), cudaMemcpyHostToDevice));
  }
