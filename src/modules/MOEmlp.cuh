...
template <typename T> class MOEMLP {
private:
  MOELinearLayer<T>
      moeLayer; // Adding a single MOE linear layer (Can add multiple in future)
...
public:
  MOEMLP(int batch_size_, int in_dim_, std::vector<int> layer_dims_,
         std::vector<int> moe_batch_splits_, bool gpu)
      : batch_size(batch_size_), in_dim(in_dim_), layer_dims(layer_dims_),
        moe_batch_splits(moe_batch_splits_), moeLayer() {
  ...
  // This function peforms the forward operation of a MLP model with a MOE layer
  // before its first layer Specifically, it should call the forward oepration
  // of each linear layer Except for the last layer, it should invoke Relu
