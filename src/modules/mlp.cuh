...
public:
  MLP(int batch_size_, int in_dim_, std::vector<int> layer_dims_, bool gpu)
    ...
    for (int i = 0; i < layer_dims.size() - 1; i++) {
      ...
      // technically, i do not need to save d_activ for backprop, but since
      // iterative training does repeated backprops, reserving space for these
      // tensor once is a good idea
  ...
  // Except for the last layer, it should invoke Relu activation after each
  // layer.
  ...
  // Tensor "d_out" is the gradients for the outputs of the last linear layer
  // (aka d_logits from op_cross_entropy_loss) Invoke the backward function of
  // each linear layer and Relu from the last one to the first one.
