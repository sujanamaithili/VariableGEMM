...
// and its gradient dt
template <typename T> class SGDFunc {
public:
  // Constructor
...
template <typename T> class AddFunc {
public:
...
template <typename T> class AddConstFunc {
public:
...
template <typename T> class MultiplyFunc {
public:
...
template <typename T> class MultiplyConstFunc {
public:
...
template <typename T> class DivideFunc {
public:
  __host__ __device__ T operator()(T x, T a) {
    // Lab-1: add your code here (delete return 0)
    //  return 0;
...
template <typename T> class DivideConstFunc {
public:
...
template <typename AT, typename BT, typename OutT> class EqualityFunc {
public:
...
template <typename T> class ReluFunc {
public:
  __host__ __device__ T operator()(T x) {
    // Lab-2: add your code here (delete return 0)
    //  return 0;
...
template <typename T> class ReluBackFunc {
public:
...
public:
  __host__ __device__ T operator()(T x) { return exp(x); }
};

// This functor implements the logarithmic operation
template <typename T> class LogFunc {
public:
  __host__ __device__ T operator()(T x) { return log(x); }
};

template <typename T> class ConstInitFunc {
public:
  __host__ __device__ T operator()(T x) { return val; }
  const float val;
};

template <typename T> class UniformInitFuncCPU {
public:
...
void op_elemwise_unary_gpu(OpFunc f, const Tensor<T> &t, Tensor<T> &out) {
  ...
  dim3 numBlocks((t.w + ELEMWISE_BLOCK_DIM - 1) / ELEMWISE_BLOCK_DIM,
                 (t.h + ELEMWISE_BLOCK_DIM - 1) / ELEMWISE_BLOCK_DIM);
...
//  "in2" only have one dimension (instead of both dimensions) in common with
//  "in1" and its other dimension has size 1. In this case, to perform elemwise
//  operation, we essentially broadcast the values of "in2" along the dimension
//  with size 1 to match the dimension size of "in1". Example1: a = [[1, 2, 3],
//  [4, 5, 6]] and b = [[10],[20]], then a+b = [[11, 12, 13], [24, 25, 26]]
...
__global__ void op_elemwise_binary_w_bcast_kernel(OpFunc f, Tensor<AT> in1,
                                                  Tensor<BT> in2,
                                                  Tensor<OutT> out) {
  // Lab-1: add your code here
  int row = blockIdx.y * blockDim.y + threadIdx.y;
...
template <typename OpFunc, typename AT, typename BT, typename OutT>
void op_elemwise_binary_w_bcast_gpu(OpFunc f, const Tensor<AT> &in1,
                                    const Tensor<BT> &in2, Tensor<OutT> &out) {
  // Lab-1: add your code here. Somewhere in this function
  // you need to call op_elemwise_binary_w_bcast_kernel<<<???, ???>>>(f, in1,
  // in2, out); delete assert(0) when you are done
  //  assert(0);
  dim3 blockSize(ELEMWISE_BLOCK_DIM, ELEMWISE_BLOCK_DIM);
  dim3 numBlocks((in1.w + ELEMWISE_BLOCK_DIM - 1) / ELEMWISE_BLOCK_DIM,
                 (in1.h + ELEMWISE_BLOCK_DIM - 1) / ELEMWISE_BLOCK_DIM);
...
template <typename T> void op_relu(const Tensor<T> &t, Tensor<T> &out) {
...
// y = exp(x)
template <typename T> void op_exp(const Tensor<T> &t, Tensor<T> &out) {
  assert(out.h == t.h && out.w == t.w);
  ExpFunc<T> f;
  if (t.on_device && out.on_device) {
    op_elemwise_unary_gpu(f, t, out);
...
// y = log(x)
template <typename T> void op_log(const Tensor<T> &t, Tensor<T> &out) {
  assert(out.h == t.h && out.w == t.w);
  LogFunc<T> f;
  if (t.on_device && out.on_device) {
    op_elemwise_unary_gpu(f, t, out);
...
template <typename T>
void op_relu_back(const Tensor<T> &in, const Tensor<T> &d_out,
                  Tensor<T> &d_in) {
...
// and stores the result in "out" tensor. lr is the learning rate. dt tensor
// should contain the gradient of parameter tensor "t".
template <typename T>
...
void op_add(const Tensor<T> &a, const Tensor<T> &b, Tensor<T> &out) {
  ...
  assert((a.h == b.h && a.w == b.w) || (a.h == b.h && b.w == 1) ||
         (a.w == b.w && b.h == 1));
  AddFunc<T> f;
...
// stores the result in tensor "out"
template <typename T> void op_add(const Tensor<T> &a, T b, Tensor<T> &out) {
  assert(out.h == a.h && out.w == a.w);
  AddConstFunc<T> f{b};
...
// stores the result in tensor "out"
template <typename T> void op_divide(const Tensor<T> &a, T b, Tensor<T> &out) {
  assert(out.h == a.h && out.w == a.w);
...
// and stores in the "out" tensor value 0 at places where "a" and "b" are not
// equal and 1 at places where "a" and "b" are equal.
...
template <typename T> void op_const_init(Tensor<T> &t, float init_val) {
...
template <typename T> void op_uniform_init(Tensor<T> &t, T min = 0, T max = 1) {
...
// This operator checks if all elements of two tensors are the "same" (aka close
// enough) with each other For now, let's settle with only CPU implementation of
// allclose
template <typename T> bool op_allclose(const Tensor<T> &at, Tensor<T> &bt) {
  ...
  for (int i = 0; i < at.h; i++) {
    for (int j = 0; j < at.w; j++) {
      // Check if the numbers are close using both relative and absolute
      // tolerances
      T a = Index(att, i, j);
      T b = Index(btt, i, j);
      if (std::abs(a - b) >
          std::max(ISCLOSE_RELTOL * std::max(std::abs(a), std::abs(b)),
                   ISCLOSE_ABSTOL)) {
        std::cout << "(" << i << "," << j << ") this=" << a << " other=" << b
                  << " diff=" << (a - b) << std::endl;
        return false;
      }
    }
