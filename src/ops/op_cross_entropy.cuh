...
template <typename T>
__global__ void
op_cross_entropy_loss_kernel(const Tensor<T> logits, const Tensor<char> targets,
                             Tensor<T> d_logits, Tensor<T> softmax,
                             Tensor<T> negative_log_softmax, Tensor<T> loss) {
  int row = blockIdx.y * blockDim.y + threadIdx.y;
  if (row < logits.h) {
    // cross entropy loss
    Index(loss, row, 0) =
        Index(negative_log_softmax, row, Index(targets, row, 0));
...
// This function calculates the cross_entropy loss from the "logits" tensor for
// a batch of training innput and the batch's corresponding "target" label
// tensor and returns the average loss of the batch. It also returns the
// gradient of the logits tensor.
template <typename T>
T op_cross_entropy_loss(const Tensor<T> &logits, const Tensor<char> &targets,
  ...
  op_cross_entropy_loss_kernel<<<blockSize, numBlocks>>>(
      logits, targets, d_logits, softmax, negative_log_softmax, loss);
