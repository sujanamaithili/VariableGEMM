...
template <typename T> __global__ void colMajor2rowMajor(T *d_A, Tensor<T> A) {
  assert(A.on_device);
  int row = threadIdx.x;
...
void op_mm(const Tensor<T> &A, const Tensor<T> &B, Tensor<T> &C) {
  ...
  *beta = 0.0;
  ...
  CUDA_CHECK(cudaMalloc((void **)&d_A, sizeof(T) * A.h * A.w));
  CUDA_CHECK(cudaMalloc((void **)&d_B, sizeof(T) * B.h * B.w));
  CUDA_CHECK(cudaMalloc((void **)&d_C, sizeof(T) * C.h * C.w));
...
int cublas_gemm(int m_array[], int n_array[], int k_array[], T alpha_array[],
  ...
  CUDA_CHECK(cudaMemcpyAsync(d_A_array, d_A.data(), sizeof(T *) * gemm_count,
                             cudaMemcpyHostToDevice, stream));
  CUDA_CHECK(cudaMemcpyAsync(d_B_array, d_B.data(), sizeof(T *) * gemm_count,
                             cudaMemcpyHostToDevice, stream));
  CUDA_CHECK(cudaMemcpyAsync(d_C_array, d_C.data(), sizeof(T *) * gemm_count,
                             cudaMemcpyHostToDevice, stream));
  CUDA_CHECK(cudaStreamSynchronize(stream));
  CUDA_CHECK(cudaDeviceSynchronize());
  /* step 3: compute */
