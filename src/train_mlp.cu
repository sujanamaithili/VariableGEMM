...
void do_one_epoch(MLP<float> &mlp, SGD<float> &sgd, Tensor<float> &images,
                  const Tensor<char> &targets, int batch_size, bool is_training,
                  int epoch_num) {
  ...
  for (int b = 0; b < images.h / batch_size; b++) {
    ...
    num_batches++;
    Tensor<float> b_images =
        images.slice(b * batch_size, (b + 1) * batch_size, 0, images.w);
    Tensor<char> b_targets =
        targets.slice(b * batch_size, (b + 1) * batch_size, 0, targets.w);
...
void train_and_test(int epochs, int batch_size, int hidden_dim, int n_layers) {
  ...
  std::cout << "# of training datapoints=" << mnist_train.images.h
            << " # of test datapoints= " << mnist_test.images.h
            << " feature size=" << mnist_train.images.w << std::endl;
  std::cout << "training datapoints mean=" << mnist_train.images.mean()
            << std::endl;
  ...
  MLP<float> mlp{batch_size, MNIST::kImageRows * MNIST::kImageColumns,
                 layer_dims, on_gpu};
  mlp.init();
